...
export default function TweedSuitPage() {
  ...
  const product = {
    ...
    description: {
      ua: 'Твідовий костюм преміальної якості',
      en: 'Premium tweed suit',
      pl: 'Garnitur tweedowy premium',
    },
    colors: ['#E8DCC8'],
  };

  const handleAddToCart = () => {
    addToCart(product, selectedSize, t.colorCream);
    setIsCartOpen(true);
  };
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4" style={{ backgroundColor: '#AEE2FC' }}>
        <div className="max-w-screen-2xl mx-auto flex items-center justify-between">
          ...
          <button onClick={() => navigate('/')} className="flex items-center gap-3 -ml-8">
            <ImageWithFallback src={logoImage} alt="SOLVIE" className="h-10 w-10 object-contain" />
